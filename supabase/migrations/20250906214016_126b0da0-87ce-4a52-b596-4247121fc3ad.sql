-- Phase 1: Critical Security Fixes - Fixed Order

-- Step 1: Add user_id to players table first
ALTER TABLE public.players ADD COLUMN user_id uuid;

-- Step 2: Add security definer helper functions (now that user_id exists)
...
-- Step 9: Add profiles trigger for new users
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Step 10: Add unique constraint to prevent duplicate players per game
